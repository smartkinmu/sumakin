...
* **年休の登録**: 年休を取得した日は「年休」にチェックを入れると始業・終業時刻が入力できなくなります。この状態では「メール作成」ボタンは無効になり、「入力確認」ボタンを押すと年休としてログに登録されます。
* **AM/PM休**: AM休を選ぶと8:30から昼休み開始時刻まで、PM休を選ぶと昼休み終了時刻から17:15まで働いたものとして入力した時刻を加算して計算します。AM休で昼休みより前の時刻、PM休で昼休みより後の時刻を入力すると警告が表示されます。年休と同様に「メール作成」ボタンは無効になり、「入力確認」ボタンを押すとログに登録されます。
...
### 4. 入力の確認と保存
* **入力確認**: 入力内容の確認と工数の自動補完を行い、その時点の内容をログに登録します。同じ日付のログが既にある場合は上書きされます。見つかった問題点は1つのダイアログにまとめて表示されます。
* **メールを作成**: 入力内容を基にメール本文を自動生成し、メールアプリを起動します（ログの登録は行いません）。
...
* 各行の「編集」ボタンを押すとポップアップが表示され、日付・始業時刻・終業時刻の編集や削除ができます。ログの上書き（入力確認による同日付の登録）・削除・編集は、直前の1回分をUNDOボタンで取り消せます。別画面に移動したり再読み込みした後でも復元できます。
* 月別集計はローカルストレージに保存されたログのみを使用して計算されます。
* ログが消失した場合は自動でバックアップから復元されます。
* 「計算チェック」ボタンから、標準休憩時間と中断時間の重なりによる勤務時間・残業時間の計算誤りを検出し、確認のうえ修正できます。修正前の状態はUNDOボタンで元に戻せます。
### バージョン管理ルール
